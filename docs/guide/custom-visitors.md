...
The parser represents `a OR b OR c` using binary `GroupNode` objects even though the user wrote no parentheses. An **implicit group** is such a node with no parentheses, field scope, prefix, negation, boost or proximity modifier. Same-operator implicit groups can be flattened into one clause list. Explicit groups such as `(a OR b)` and custom queries remain separate conditions.
