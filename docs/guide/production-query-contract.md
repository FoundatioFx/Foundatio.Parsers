...
Use this version 1 acceptance suite to check document matching, scoring, and query migrations against a reproducible mapping profile. It asserts expected results independently of the parser, including required clauses, literal wildcards, nested correlation, and date boundaries. A failing assertion needs investigation; changing an expected result to match an implementation defect does not establish compatibility.
...
| Engine matrix | Run separately on Elasticsearch 8.19.0 and 9.5.0 and record the exact server/Lucene versions with each result |
...
Unknown fields, arbitrary mappings and custom visitors are **not rejected merely because they are outside this profile**. The suite does not change parser behavior or add a public API. Applications must separately enforce their field, operation and cost policy.
...
| Rejected input exercised | Leading wildcard operators under the configured policy, escaped dots, post-colon term/range operators, unsupported edit distances, fractional phrase slop and negative boosts must fail before search |
...
`production-migrations.tsv` executes pairs of accepted expressions and asserts an independent expected document set, rather than accepting two equally wrong answers. Separate rejection tests cover legacy post-colon operators; the leading-negation pair verifies the replacement query's results.
...
| Legacy `message:-alpha`, `message:+alpha`, or `message:NOT alpha` | These forms are rejected. Move the operator before the field: `-message:alpha`, `+message:alpha`, or `NOT message:alpha`. See [operator-placement migration](./query-syntax#breaking-syntax-correction-operator-placement) for groups and literal values |
...
The required-clause and term-modifier fixes change results for previously ignored required clauses and modifiers, and change scores where boosts were ignored. **Do not preserve those incorrect outputs for backward compatibility without a deliberate product decision.** Before deploying, replay the application's saved-query corpus, identify affected users/configurations, and communicate the behavior changes. These tests do not provide a legacy-mode flag or migrate stored queries automatically.
...
From the repository root, set `ELASTICSEARCH_URL` to an intended disposable Elasticsearch server. The fixture creates isolated indexes and deletes them during cleanup. Repeat these commands for each engine version, saving each run's output separately:

```bash
mkdir -p contract-results
git rev-parse HEAD 'HEAD^{tree}' > contract-results/revision.txt
curl --fail --silent --show-error "${ELASTICSEARCH_URL:?Set a disposable test server}" > contract-results/elasticsearch.json
shasum -a 256 tests/compatibility/production-* > contract-results/input-sha256.txt
...
dotnet tests/Foundatio.Parsers.ElasticQueries.Tests/bin/Release/net10.0/Foundatio.Parsers.ElasticQueries.Tests.dll \
  ...
  -result-xml contract-results/contract.xml
...
The tests participate in the normal solution suite; this change adds no workflow or automatic two-version matrix. For a reproducible comparison, retain the revision, any local diff, the profile/corpora and their hashes, server versions, XML and command output for each run. XML contains query diagnostics. The standalone report verifier requires every matching/migration case in all three operator configurations and both scoring modes, and every date case in both scoring modes. It rejects missing/duplicate case configurations, invalid expected document IDs, wrong test classes, skips, assembly errors and inconsistent counts. Its own mutation tests include equal-count substitution, so a passing total alone is insufficient.

The current inventory is 45 matching rows × 6 configurations, 8 migration rows × 6, 11 date rows × 2, and 39 additional test cases: **379 xUnit executions per server**. Some additional cases internally exercise both contexts. Counts are execution inventory, not 379 distinct syntax features and not an exhaustive compatibility claim.

## Release and maintenance gate

Run the full solution tests, documentation checks and both engine versions on the revision being evaluated. A previous passing revision is not evidence for a new tree. This suite adds no library behavior changes; review the [syntax compatibility](./syntax-compatibility), [query syntax](./query-syntax), and [ordering validation](./validation#ordering-operators) guides for the merged behavior it exercises.
