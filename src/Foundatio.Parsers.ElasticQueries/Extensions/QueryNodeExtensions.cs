...
public static class QueryNodeExtensions
{
    ...
    public static FieldSort GetSort(this IQueryNode node, Func<FieldSort> getDefaultValue = null)
    {
        if (!node.Data.TryGetValue(SortKey, out object value))
            return getDefaultValue?.Invoke();

        return value as FieldSort;
    }

    public static void SetSort(this IQueryNode node, FieldSort sort)
    {
